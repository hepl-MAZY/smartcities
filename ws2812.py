...
def ws2812():
    ...
    label("bitloop")
    out(x, 1) .side(0) [T3 - 1]  # Output the bit
    jmp(not_x, "do_zero") .side(1) [T1 - 1]  # If x == 0, go to do_zero
    jmp("bitloop") .side(1) [T2 - 1]  # Otherwise, continue the loop
    label("do_zero")
    nop() .side(0) [T2 - 1]  # Send the "0" bit
